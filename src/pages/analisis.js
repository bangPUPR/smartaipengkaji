...
function renderResultPanel(result, proyek) {
  const aspekDefs = [
    ...
    { key: 'skor_struktur',     label: 'Struktur/Safety', icon: 'fa-building',           color: 'hsl(0,70%,55%)',   kpiColor: 'kpi-red'    },
    { key: 'skor_mekanikal',    label: 'Mekanikal/MEP', icon: 'fa-bolt',               color: 'hsl(40,80%,55%)',  kpiColor: 'kpi-yellow' },
...
window._runAspect = async (aspekTarget) => {
  try {
    showAIProgress(`Menganalisis: ${aspekTarget}`, `OpenAI Engineer sedang mengevaluasi data ${aspekTarget}`);
    ...
    // 2. Berikan ke AI
    const result = await runAspectAnalysis(aspekTarget, checklistData, (current, total, itemName) => {
      showAIProgress(
        `Menganalisis: ${aspekTarget} (${current}/${total})`, 
        `Mengevaluasi teknis: ${itemName}...`
      );
    });
    ...
    const mapKolom = {
      ...
      'Pemanfaatan': 'skor_pemanfaatan',
      'Arsitektur': 'skor_arsitektur',
      'Struktur': 'skor_struktur',
      'Mekanikal': 'skor_mekanikal',
      ...
      data: {
        ...
        datasets: [{
          ...
          data: [
            ...
            result.skor_pemanfaatan,
            result.skor_arsitektur,
            result.skor_struktur,
            result.skor_mekanikal,
